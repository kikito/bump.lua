  ...
  describe(':getTouch', function()
    ...
    describe('on tunnels', function()
      it('returns the coordinates of the item when it starts touching the other, and the normal', function()
        assert.same({resolve(box( -3,  3,2,2), other, 3,3):getTouch()}, { -2,  3, -1,  0})
  ...
  describe(':getSlide', function()
    ...
    describe('when there is movement, it slides', function()
      it('slides on intersections', function()
        assert.same({resolve(box( 3, 3,2,2), other, 4, 5):getSlide()}, { 0.5, -2, 0,-1, 4, -2})
        assert.same({resolve(box( 3, 3,2,2), other, 5, 4):getSlide()}, { -2, 0.5, -1,0, -2, 4})
        assert.same({resolve(box( 3, 3,2,2), other, 2, 1):getSlide()}, { 5.5, 8, 0,1, 2, 8})
        assert.same({resolve(box( 3, 3,2,2), other, 1, 2):getSlide()}, { 8, 5.5, 1,0, 8, 2})
      end)

      it('slides over tunnels', function()
        assert.same({resolve(box(10,10,2,2), other, 1, 4):getSlide()}, { 7, 8, 0, 1, 1, 8})
        assert.same({resolve(box(10,10,2,2), other, 4, 1):getSlide()}, { 8, 7, 1, 0, 8, 1})

        -- perfect corner case:
        assert.same({resolve(box(10,10,2,2), other, 1, 1):getSlide()}, { 8, 8, 1, 0, 8, 1})
      end)
    end)
  end)
  ...
  describe(':getBounce', function()
    ...
    describe('when there is movement, it bounces', function()
      it('bounces on intersections', function()
        assert.same({resolve(box( 3, 3,2,2), other, 4, 5):getBounce()}, { 0.5, -2, 0,-1, 4, -9})
        assert.same({resolve(box( 3, 3,2,2), other, 5, 4):getBounce()}, { -2, 0.5, -1,0, -9, 4})
        assert.same({resolve(box( 3, 3,2,2), other, 2, 1):getBounce()}, { 5.5, 8, 0,1, 2, 15})
        assert.same({resolve(box( 3, 3,2,2), other, 1, 2):getBounce()}, { 8, 5.5, 1,0, 15,2})
      end)

      it('bounces over tunnels', function()
        assert.same({resolve(box(10,10,2,2), other, 1, 4):getBounce()}, { 7, 8, 0, 1, 1, 12})
        assert.same({resolve(box(10,10,2,2), other, 4, 1):getBounce()}, { 8, 7, 1, 0, 12, 1})

        -- perfect corner case:
        assert.same({resolve(box(10,10,2,2), other, 1, 1):getBounce()}, { 8, 8, 1, 0, 15, 1})
      end)
    end)
  end)
