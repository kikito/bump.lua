...
describe('World', function()
  ...
  describe(':getBounce', function()
    ...
    describe('when there is no movement', function()
      it('behaves like :getTouch(), plus safe info', function()
        local c = resolve(box(3,3,2,2), other)
        assert.same({c:getBounce()}, {3,8, 0,1, 3,8, 0,0})
      end)
    end)
    describe('when there is movement, it bounces', function()
      it('bounces on intersections', function()
        assert.same({resolve(box( 3, 3,2,2), other, 4, 5):getBounce()}, { 0.5, -2, 0,-1, 4, -9, 3.5, -7})
        assert.same({resolve(box( 3, 3,2,2), other, 5, 4):getBounce()}, { -2, 0.5, -1,0, -9, 4, -7, 3.5})
        assert.same({resolve(box( 3, 3,2,2), other, 2, 1):getBounce()}, { 5.5, 8, 0,1, 2, 15, -3.5, 7})
        assert.same({resolve(box( 3, 3,2,2), other, 1, 2):getBounce()}, { 8, 5.5, 1,0, 15,2, 7, -3.5})
      end)
