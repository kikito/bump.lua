  ...
  describe(":resolve", function()
    describe('when there are no collisions', function()
      it('it moves the object, and returns zero collisions', function()
        ...
        assert.same({1,1,{},0}, {world:resolve(item, 1,1)})
      end)
    end)
    ...
    describe('when touching', function()
      it('returns a collision with the first item it touches', function()
        ...
        local b  = world:add('b', 0,2,1,1)
        local c  = world:add('c', 0,2,1,1)
        local d  = world:add('d', 0,3,1,1)
        local x,y,cols,len = world:resolve(a, 0,5)
        assert.same({x,y}, {0,1})
        assert.equals(1, len)
        assert.same(collect(cols, 'other'), {'b'})
        assert.same(collect(cols, 'kind'),  {'touch'})
        assert.same({x=0,y=1,w=1,h=1}, world:getRect(a))
      end)
    end)
    ...
    describe('when crossing', function()
      it('returns a collision with every item it crosses', function()
        ...
        local c  = world:add('c', 0,2,1,1)
        local d  = world:add('d', 0,3,1,1)
        local x,y,cols,len = world:resolve(a, 0,5, function() return 'cross' end)
        assert.same({x,y}, {0,5})
        assert.equals(3, len)
        assert.same(collect(cols, 'other'), {'b', 'c', 'd'})
        assert.same(collect(cols, 'kind'),  {'cross', 'cross', 'cross'})
        assert.same({x=0,y=5,w=1,h=1}, world:getRect(a))
      end)
    end)
    ...
    describe('when sliding', function()
      it('slides with every element', function()
        ...
        local c  = world:add('c', 2,1,1,1)
        local x,y,cols,len = world:resolve(a, 5,5, function() return 'slide' end)
        assert.same({x,y}, {1,1})
        assert.equals(2, len)
        assert.same(collect(cols, 'other'), {'b','c'})
        assert.same(collect(cols, 'kind'),  {'slide','slide'})
        assert.same({x=1,y=1,w=1,h=1}, world:getRect(a))
      end)
    end)
    ...
    describe('when bouncing', function()
      it('bounces on each element',function()
        local a  = world:add('a', 0,0,1,1)
        local b  = world:add('b', 0,2,1,2)
        local x,y,cols,len = world:resolve(a, 0,5, function() return 'bounce' end)
        assert.same({x,y}, {0,-3})
        assert.same(collect(cols, 'other'), {'b'})
        assert.same(collect(cols, 'kind'),  {'bounce'})
        assert.same({x=0,y=-3,w=1,h=1}, world:getRect(a))
      end)
    end)
  end)
end)
