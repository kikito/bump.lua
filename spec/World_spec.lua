...
describe('World', function()
  ...
  describe(":resolve", function()
    ...
    describe('when touching', function()
      ...
      it('returns collisions with all the *first* items it touches simultaneously', function()
        local a  = world:add('a', 0,0,1,1)
        local b  = world:add('b', 0,2,1,1)
        ...
        assert.same({x,y}, {0,1})
        assert.equals(2, len)
        assert.same(sorted(collect(cols, 'other')), {'b', 'c'})
        assert.same(collect(cols, 'kind'),  {'touch','touch'})
      end)
    end)
  end)
end)
